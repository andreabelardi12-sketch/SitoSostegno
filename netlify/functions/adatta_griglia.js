exports.handler = async function(event, context) {
    ...
    try {
        ...
        if (!apiKey) return { statusCode: 500, body: JSON.stringify({ error: "Chiave API mancante" }) };
        if (!body.profilo || !body.griglia) return { statusCode: 400, body: JSON.stringify({ error: "Campi mancanti" }) };
        ...
        if (!data.candidates?.length) throw new Error("Nessuna risposta generata");
